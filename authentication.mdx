...
const accounts = await client.getAccountsDetails();
...
| `customSignMessageFn` | `function` | Provide a custom HMAC signing function (e.g. Node's `createHmac` for better performance). |
