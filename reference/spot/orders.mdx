...
<ParamField body="stptype" type="'cancel-newest' | 'cancel-oldest' | 'cancel-both'">
  Self-trade prevention behaviour when both sides of a trade would belong to the same account.
  `cancel-newest` cancels the incoming order; `cancel-oldest` cancels the resting order; `cancel-both` cancels both.
</ParamField>
